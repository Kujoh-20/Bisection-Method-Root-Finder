...
ACCENT2   = "#ffd54f"
ACCENT3   = "#69f0ae"
ACCENT4   = "#ff8a65"   # orange — False Position highlight
TEXT_FG   = "#e8eaf6"
MUTED     = "#7986cb"
...
TRAIL_WARN    = "#ff7043"
TRAIL_DIM     = "#546e7a"
TRAIL_METHOD  = "#ff8a65"   # orange — method badge
...
def run_bisection(f, a, b, tol, max_iter):
    ...
    c = (a + b) / 2  — always the geometric midpoint.
    """
    history, brackets = [], []
    ca, cb = a, b
...
def run_false_position(f, a, b, tol, max_iter):
    ...
    c = b - f(b)*(b-a)/(f(b)-f(a))  — weighted by function values.
    ...
    for i in range(1, max_iter + 1):
        fa, fb = float(f(ca)), float(f(cb))
        # False position formula — intersection of secant with x-axis
        denom = fb - fa
        if abs(denom) < 1e-15:
...
def build_trail_segments(method_name, f_expr, a, b, tol, max_iter,
    ...
    for (i, ia, ib, fa, fb, ic, fc, w, dec) in history:
        ...
        # Show method-specific midpoint formula
        if method_name == "Bisection":
...
def compute():
    ...
    method_name = method_var.get()   # "Bisection" or "False Position"
    ...
    try:
        ...
        return
    if fa * fb > 0:
        messagebox.showerror("Bracket Error",
            f"f({a}) = {fa:.4f} and f({b}) = {fb:.4f} have the SAME sign.\n"
            "Choose an interval where f(a) and f(b) have opposite signs.")
        for tb in [steps_text, results_text]: tb.config(state=DISABLED)
        return
    ...
    trail_segs = build_trail_segments(
        method_name, f_expr, a, b, tol, max_iter, history, root_val, stop_reason)
    ...
    notebook.select(1)
    update_status(f"[{method_name}]  Root = {root_val:.8f}  |  {n} iterations  |  Ready")
...
def clear_all():
    ...
    steps_text.insert(END,
        ...
        "It will stream line-by-line, showing exactly how the chosen\n"
        "method reasons through every iteration.\n\n"
        "Select a method above, enter your function, then compute.\n",
        "explain")
...
def show_about():
    """About / Help popup dialog (scrollable, no overlap)."""
    ...
    def section(title, y_pos):
        ...
        canvas.create_line(30, y_pos+18, w-30, y_pos+18,
                           fill="#2a2a4a", width=1)
    ...
    # Project Info
    section("PROJECT INFO", y); y += 35
    row("Project:", "Bisection Method Root Finder", y); y += 28
    row("Version:", "v1.0", y); y += 28
    row("Course:", "Numerical Analysis", y); y += 40

    # Members
    section("GROUP MEMBERS", y); y += 35
    for name in [
        "John Oniel Thomas Q. Araque",
        "Niel Allen S. Jauculan",
        "Tiff Anthony R. Parnala"
    ]:
        canvas.create_text(30, y, text=name,
    ...
    # Description
    section("DESCRIPTION", y); y += 35
    desc = (
        "This system finds roots of mathematical functions using two "
        "bracketing methods: Bisection and False Position (Regula Falsi).\n\n"
        "Both methods require an interval [a, b] where f(a) and f(b) have opposite signs, guaranteeing a root exists between them"
        " by the Intermediate Value Theorem.\n\n"
        "The app streams a step-by-step Solution Trail showing every "
        "decision the algorithm makes, plots the function graph, and displays a full iteration table."
    )

    desc_id = canvas.create_text(30, y, text=desc,
                                 anchor="nw",
                                 fill=MUTED,
                                 font=("Segoe UI", 9),
                                 width=460)
    ...
    desc_box = canvas.bbox(desc_id)
    y = desc_box[3] + 25   # ✅ move below description

    # How to Use
    section("HOW TO USE", y); y += 30
    steps = (
        "1. Select a method (Bisection or False Position)\n"
    ...
    steps_id = canvas.create_text(30, y, text=steps,
                                  anchor="nw",
                                  fill=TEXT_FG,
                                  font=("Segoe UI", 9),
                                  width=460)
    ...
    canvas.update_idletasks()
    steps_box = canvas.bbox(steps_id)
    y = steps_box[3] + 40   # ✅ move below steps
    ...
    canvas.create_window(w//2, y, window=close_btn)

    # Final scroll region
    canvas.configure(scrollregion=(0, 0, w, y + 60))
...
def make_method_btn(parent, label, value, desc):
    """Radio-button style method selector card."""
...
# Each entry: (display label, f(x) expr, a, b, description)
test_cases = [
    ("TC1: x^3-x-2",    "x**3 - x - 2", "1",  "2",
     "Cubic polynomial — classic test"),
    ("TC2: cos(x)-x",   "cos(x) - x",   "0",  "1",
     "Transcendental — Dottie number"),
    ("TC3: x^2-4",      "x**2 - 4",     "1",  "3",
     "Quadratic — root at x=2"),
    ("TC4: exp(x)-3x",  "exp(x) - 3*x", "0",  "1",
     "Exp vs linear — smooth curve"),
    ("TC5: sin(x)",     "sin(x)",        "2",  "4",
     "Trig function — root at pi"),
]

for lbl, expr, ea, eb, desc in test_cases:
    row = tk.Frame(scroll_inner, bg=CARD_BG)
...
steps_text.insert(END,
    ...
    "  -> Method identification badge\n"
    "  -> IVT validity check\n"
    "  -> Method-specific midpoint formula shown\n"
    "  -> f(c) evaluation\n"
