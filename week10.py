...
TRAIL_VERIFY  = "#b39ddb"   # soft violet — verification section
...
# ════════════════════════════════════════════
#  VERIFICATION ENGINE
#  Runs after every computation — no exceptions.
#  Returns a dict with all verification metrics.
# ════════════════════════════════════════════
def verify_root(f_expr, root_val, a, b, tol, n_iters):
    """
    Performs four independent checks on the computed root and
    returns a verification report dictionary.

    Checks
    ------
    1. Numeric Residual     — f(root) should be ~0
    2. Back-substitution    — substitute root back into f(x) and evaluate
    3. Bracket Containment  — root must lie inside original [a, b]
    4. Theoretical Error    — (b-a)/2^n gives guaranteed error bound
    5. Residual vs Tol      — |f(root)| compared to user tolerance
    """
    f = make_f(f_expr)
    report = {}

    # Check 1 — Numeric residual
    try:
    ...
    # Check 2 — Back-substitution (same as residual but stated differently)
    report["backcheck_val"]  = report.get("residual")
    report["backcheck_ok"]   = report.get("residual_ok", False)

    # Check 3 — Bracket containment
    report["in_bracket"]     = (a <= root_val <= b)

    # Check 4 — Theoretical error bound
    if n_iters > 0:
    ...
    # Overall pass/fail
    report["passed"] = (
        report["residual_ok"] and
        report["in_bracket"] and
        report["error_bound_ok"]
    )
...
def build_trail_segments(method_name, f_expr, a, b, tol, max_iter,
    ...
    def v(t):    segs.append((t + "\n", "verify"))    # verification line
    def lv(label, val):
    ...
    #  Runs 5 independent checks every run.
    # ══════════════════════════════════════
    vr = verify_root(f_expr, root_val, a, b, tol, n)

    h("=" * 56)
    h("  VERIFICATION REPORT")
    h("=" * 56)
    blank()
    v("  PURPOSE: Independently confirm the computed root is correct")
    v("  by substituting it back into f(x) and running 5 checks.")
    blank()

    # Check 1 — Numeric Residual
    v("  CHECK 1 — NUMERIC RESIDUAL")
    v("  " + "-" * 50)
    v("  Substitute root back: compute f(root) directly.")
    v(f"  root  = {root_val:.12f}")
    v(f"  f(root) = f({root_val:.8f})")
    if vr["residual"] is not None:
        v(f"         = {vr['residual']:.6e}")
        v(f"  |f(root)| = {vr['residual_abs']:.6e}")
        if vr["residual_ok"]:
            segs.append(("  PASS — residual is negligibly small.\n", "good"))
        else:
            segs.append(("  WARNING — residual is larger than expected.\n", "warn"))
    else:
        segs.append(("  ERROR — could not evaluate f(root).\n", "warn"))
    blank()

    # Check 2 — Back-Substitution
    v("  CHECK 2 — BACK-SUBSTITUTION")
    v("  " + "-" * 50)
    v("  Re-evaluate f(x) at the found root independently.")
    v("  This confirms the root satisfies the original equation.")
    if vr["backcheck_val"] is not None:
        v(f"  f({root_val:.8f}) = {vr['backcheck_val']:.10f}")
        v(f"  Expected: ~0.0  |  Got: {vr['backcheck_val']:.2e}")
        if vr["backcheck_ok"]:
            segs.append(("  PASS — back-substitution confirms the root.\n", "good"))
        else:
            segs.append(("  WARNING — back-substitution shows large deviation.\n", "warn"))
    blank()

    # Check 3 — Bracket Containment
    v("  CHECK 3 — BRACKET CONTAINMENT")
    v("  " + "-" * 50)
    v(f"  Root must lie inside the original interval [{a}, {b}].")
    v(f"  root = {root_val:.12f}")
    v(f"  {a} <= {root_val:.8f} <= {b}  ?")
    if vr["in_bracket"]:
        segs.append(("  PASS — root is inside the original bracket.\n", "good"))
    else:
        segs.append(("  FAIL — root is OUTSIDE the original bracket!\n", "warn"))
    blank()

    # Check 4 — Theoretical Error Bound
    v("  CHECK 4 — THEORETICAL ERROR BOUND")
    v("  " + "-" * 50)
    v("  Formula:  error <= (b - a) / 2^n")
    v(f"  n = {n} iterations")
    v(f"  (b - a) / 2^{n} = ({b} - {a}) / {2**n}")
    v(f"             = {vr['error_bound']:.6e}")
    v(f"  Tolerance  = {tol}")
    if vr["error_bound_ok"]:
        segs.append(("  PASS — error bound is within acceptable range.\n", "good"))
    else:
        segs.append(("  NOTE — error bound exceeds tolerance (consider more iterations).\n", "warn"))
    blank()

    # Check 5 — Residual vs Tolerance
    v("  CHECK 5 — RESIDUAL vs TOLERANCE")
    v("  " + "-" * 50)
    v("  Direct comparison: is |f(root)| < tolerance?")
    if vr["residual_abs"] is not None:
        v(f"  |f(root)| = {vr['residual_abs']:.6e}")
        v(f"  Tolerance = {tol}")
        if vr["within_tol"]:
            v(f"  {vr['residual_abs']:.2e}  <  {tol}  TRUE")
            segs.append(("  PASS — residual is within user tolerance.\n", "good"))
        else:
            v(f"  {vr['residual_abs']:.2e}  >=  {tol}  (bracket rule stopped it)")
            segs.append(("  NOTE — bracket tolerance fired; f(root) may be slightly above tol.\n", "warn"))
    blank()

    # Overall verdict
    v("  " + "=" * 50)
    if vr["passed"]:
        segs.append(("  VERIFICATION RESULT:  ALL CHECKS PASSED\n", "good"))
        segs.append(("  The computed root is verified correct.\n", "good"))
    else:
        segs.append(("  VERIFICATION RESULT:  ONE OR MORE CHECKS FLAGGED\n", "warn"))
        segs.append(("  Review warnings above. Result may need better settings.\n", "warn"))
    v("  " + "=" * 50)
    blank()
...
# ════════════════════════════════════════════
#  EXPORT ENGINE
#  Builds TXT and HTML reports from last run.
#  Sections: GIVEN / METHOD / STEPS / FINAL /
#            VERIFICATION / SUMMARY
# ════════════════════════════════════════════

# Holds all data from the last successful run so export can access it.
_last_run = {}

def build_txt_report(d):
    """Build a plain-text report string from run data dict `d`."""
    ...
    def sec(title): bar(); lines.append(f"  {title}"); bar()

    bar("=")
    lines.append("  BISECTION METHOD ROOT FINDER")
    ...
    if d['method_name'] == "Bisection":
        ...
        ln("                 Guaranteed linear convergence.")
    else:
        ln("  Formula      : c = b - f(b)*(b-a) / (f(b)-f(a))")
        ln("  Description  : Uses a weighted secant line to estimate the root.")
        ln("                 Typically converges faster than bisection.")
    ln()
    ln("  Stopping Rules:")
    ln(f"    Rule 1 — |f(c)| < {d['tol']}  (function value tolerance)")
    ln(f"    Rule 2 — (b-a)/2 < {d['tol']}  (bracket width tolerance)")
    ln(f"    Rule 3 — iterations >= {d['max_iter']}  (safety cap)")
    ln(f"  Active Rule  : {d['stop_label']}")
    ln()

    # ── SECTION 3: STEPS (Iteration Table) ──
    sec("SECTION 3 — ITERATION STEPS")
    ln(f"  {'Iter':>4}  {'a':>14}  {'b':>14}  {'c (midpoint)':>16}  {'f(c)':>14}  {'Decision':>12}")
    ln("  " + "-" * 80)
    ...
    ln(f"    Result    : {'PASS' if vr['residual_ok'] else 'WARN'} — "
       + ("negligibly small." if vr['residual_ok'] else "larger than expected."))
    ln()
    ...
    ln(f"    Tolerance = {d['tol']}")
    ln(f"    Result    : {'PASS' if vr['error_bound_ok'] else 'NOTE — exceeds tol'}")
    ln()
    ln("  Check 5 — Residual vs Tolerance")
    ln(f"    |f(root)| = {res_abs}  vs  tol = {d['tol']}")
    ln(f"    Result    : {'PASS' if vr['within_tol'] else 'NOTE — bracket rule fired'}")
    ln()
...
def build_html_report(d):
    """Build a styled HTML report string from run data dict `d`."""
    from datetime import datetime
    now = datetime.now().strftime("%Y-%m-%d  %H:%M:%S")
    vr = d['vr']
    res_abs = f"{vr['residual_abs']:.6e}" if vr['residual_abs'] is not None else "N/A"
    ...
    eb_str  = f"{vr['error_bound']:.4e}"
    verdict = "ALL CHECKS PASSED" if vr['passed'] else "ONE OR MORE CHECKS FLAGGED"
    verdict_color = "#69f0ae" if vr['passed'] else "#ff7043"

    def pass_badge(ok, pass_label="PASS", fail_label="WARN"):
        c = "#69f0ae" if ok else "#ff7043"
        label = pass_label if ok else fail_label
        return f'<span style="color:{c};font-weight:bold">{label}</span>'

    rows_html = ""
    for (i, ia, ib, fa, fb, ic, fc, w, dec) in d['history']:
        bg = "#1a1a2e" if i % 2 == 0 else "#12121e"
        rows_html += (
    ...
    edge_html = ""
    if d.get('edge_warnings'):
        edge_html = '<div class="section"><h2>Edge Case Warnings</h2>'
        for code, msg in d['edge_warnings']:
            edge_html += f'<p style="color:#ff7043"><strong>[{code}]</strong> {msg}</p>'
        edge_html += '</div>'
...
<style>
  body {{ font-family: 'Consolas', monospace; background:#0d0d18; color:#e8eaf6; margin:0; padding:24px; }}
  h1   {{ color:#4fc3f7; border-bottom:2px solid #2a2a4a; padding-bottom:8px; }}
  h2   {{ color:#ffd54f; margin-top:32px; border-left:4px solid #ffd54f; padding-left:10px; }}
  .card{{ background:#1c1c2e; border-radius:8px; padding:18px 24px; margin:16px 0; }}
  .meta{{ color:#7986cb; font-size:0.9em; }}
  table{{ width:100%; border-collapse:collapse; font-size:0.88em; margin-top:10px; }}
  th   {{ background:#1c1c2e; color:#ffd54f; padding:8px; text-align:center; }}
  td   {{ padding:6px 8px; text-align:center; color:#c8c8ff; }}
  .pass{{ color:#69f0ae; font-weight:bold; }}
  .warn{{ color:#ff7043; font-weight:bold; }}
  .root{{ color:#4fc3f7; font-size:1.4em; font-weight:bold; }}
  .verdict{{ font-size:1.1em; font-weight:bold; color:{verdict_color}; padding:10px;
             border:2px solid {verdict_color}; border-radius:6px; display:inline-block; margin-top:10px; }}
  .section {{ margin-bottom: 28px; }}
  label {{ color:#c792ea; display:inline-block; width:160px; }}
</style>
</head>
<body>

<h1>Bisection Method Root Finder — Audit Report</h1>
<p class="meta">Generated: {now} &nbsp;|&nbsp; Version: v1.0</p>
<p class="meta">Group: John Oniel Thomas Q. Araque &nbsp;|&nbsp; Niel Allen S. Jauculan &nbsp;|&nbsp; Tiff Anthony R. Parnala</p>

<!-- SECTION 1: GIVEN -->
<div class="section">
<h2>Section 1 — Given</h2>
<div class="card">
  ...
  <p><label>IVT Valid:</label> {'<span class="pass">YES — opposite signs</span>' if d['fa0']*d['fb0']<0 else '<span class="warn">NO — same sign</span>'}</p>
</div>
</div>

<!-- SECTION 2: METHOD -->
<div class="section">
<h2>Section 2 — Method Used</h2>
<div class="card">
  <p><label>Method:</label> <strong style="color:#ff8a65">{d['method_name']}</strong></p>
  {'<p><label>Formula:</label> c = (a + b) / 2</p><p>Splits the bracket exactly in half each iteration. Guaranteed linear convergence.</p>' if d['method_name']=="Bisection" else '<p><label>Formula:</label> c = b − f(b)·(b−a) / (f(b)−f(a))</p><p>Weighted secant line approximation. Often faster than bisection on smooth functions.</p>'}
...
<!-- SECTION 3: STEPS -->
<div class="section">
<h2>Section 3 — Iteration Steps</h2>
<div class="card">
<table>
...
<!-- SECTION 5: VERIFICATION -->
<div class="section">
<h2>Section 5 — Verification Report</h2>
<div class="card">
  <table>
    <tr><th>Check</th><th>Name</th><th>Value</th><th>Result</th></tr>
    <tr style="background:#1a1a2e"><td>1</td><td>Numeric Residual</td><td>|f(root)| = {res_abs}</td><td>{pass_badge(vr['residual_ok'])}</td></tr>
    <tr style="background:#12121e"><td>2</td><td>Back-Substitution</td><td>f(root) = {res_val}</td><td>{pass_badge(vr['backcheck_ok'])}</td></tr>
    <tr style="background:#1a1a2e"><td>3</td><td>Bracket Containment</td><td>{d['a']} ≤ root ≤ {d['b']}</td><td>{pass_badge(vr['in_bracket'])}</td></tr>
    <tr style="background:#12121e"><td>4</td><td>Error Bound</td><td>(b−a)/2<sup>n</sup> = {eb_str}</td><td>{pass_badge(vr['error_bound_ok'],'PASS','NOTE')}</td></tr>
    <tr style="background:#1a1a2e"><td>5</td><td>Within Tolerance</td><td>|f(r)| &lt; tol ?</td><td>{pass_badge(vr['within_tol'],'PASS','NOTE')}</td></tr>
  </table>
...
<div class="card">
  ...
  <p><label>Verification:</label> <span style="color:{verdict_color};font-weight:bold">{verdict}</span></p>
</div>
</div>
...
def export_report():
    """Open save dialog and write TXT or HTML report."""
    ...
    btn_row = tk.Frame(fmt_win, bg=CARD_BG)
    btn_row.pack()
    tk.Radiobutton(btn_row, text="Plain Text (.txt)", variable=fmt_var, value="txt",
                   bg=CARD_BG, fg=ACCENT1, selectcolor=BG,
                   activebackground=CARD_BG, activeforeground=ACCENT1,
                   font=("Segoe UI", 10)).pack(side=LEFT, padx=12)
    tk.Radiobutton(btn_row, text="HTML (.html)", variable=fmt_var, value="html",
                   bg=CARD_BG, fg=ACCENT1, selectcolor=BG,
                   activebackground=CARD_BG, activeforeground=ACCENT1,
                   font=("Segoe UI", 10)).pack(side=LEFT, padx=12)
    ...
    def do_export():
        ...
        if fmt == "txt":
            ...
            ftype = [("HTML files", "*.html"), ("All files", "*.*")]

        path = fd.asksaveasfilename(
            defaultextension=f".{fmt}",
            filetypes=ftype,
            initialfile=fn_default,
        ...
        try:
            ...
            with open(path, "w", encoding="utf-8") as fh_:
                fh_.write(content)
            update_status(f"Report exported: {path.split('/')[-1].split(chr(92))[-1]}")
            messagebox.showinfo("Export Successful",
                f"Report saved successfully!\n\n"
                f"File: {path}\n\n"
                f"The report contains:\n"
                f"  Section 1 — Given (inputs)\n"
                f"  Section 2 — Method used\n"
                f"  Section 3 — Iteration steps\n"
                f"  Section 4 — Final answer\n"
                f"  Section 5 — Verification report\n"
                f"  Section 6 — Summary")
        except Exception as exc:
            messagebox.showerror("Export Failed", f"Could not save file:\n{exc}")

    tk.Button(fmt_win, text="  Export  ",
              bg=ACCENT1, fg="#000000",
              font=("Segoe UI", 10, "bold"),
              relief="flat", bd=0, padx=16, pady=6,
              cursor="hand2", command=do_export
              ).pack(pady=14)
...
def compute():
    ...
    _last_run.update({
        ...
        "max_iter"     : max_iter,
        "root_val"     : root_val,
        "f_root"       : float(f_obj(root_val)),
        "n"            : n,
        "err_b"        : (b - a) / (2 ** n),
        "history"      : history,
        "stop_reason"  : stop_reason,
        "stop_label"   : (f"Yes  (Rule 1: |f(c)| < {tol})" if stop_reason == "f(c)_tol"
                          else f"Yes  (Rule 2: bracket/2 < {tol})" if stop_reason == "bracket_tol"
                          else f"No   (Rule 3: hit max iter = {max_iter})"),
        "vr"           : vr,
        "edge_warnings": all_warnings if all_warnings else [],
    })
    if all_warnings:
        edge_summary = "\n  EDGE CASE WARNINGS:\n"
    ...
    v_lines  = (
        ...
        f"  {'-'*40}\n"
        f"  Check 1  Residual     |f(root)|  = {vr['residual_abs']:.4e}  {'PASS' if vr['residual_ok'] else 'WARN'}\n"
        f"  Check 2  Back-sub     f(root)    = {res_str}           {'PASS' if vr['backcheck_ok'] else 'WARN'}\n"
        f"  Check 3  In bracket   [{a},{b}]  {'PASS' if vr['in_bracket'] else 'FAIL'}\n"
        f"  Check 4  Error bound  (b-a)/2^n  = {eb_str}  {'PASS' if vr['error_bound_ok'] else 'NOTE'}\n"
        f"  Check 5  Within tol   |f(r)|<tol {'PASS' if vr['within_tol'] else 'NOTE'}\n"
        f"  {'-'*40}\n"
        f"  RESULT: {v_verdict}\n"
    )
...
export_btn = tk.Button(hdr, text=" Export Report ",
                       bg="#1a3a1a", fg=ACCENT3,
                       font=("Segoe UI", 9, "bold"),
                       relief="flat", bd=0, padx=10, pady=4,
                       cursor="hand2", activebackground="#2a552a",
                       activeforeground=ACCENT3, command=export_report)
export_btn.place(relx=0.0, rely=0.5, anchor="w", x=140)
...
steps_text.tag_configure("verify",  foreground=TRAIL_VERIFY,  font=("Consolas", 10))
